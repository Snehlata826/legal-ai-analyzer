"""
Risk analysis using keyword matching — used as fallback when ML model unavailable.
"""
import re
...
MEDIUM_RISK_KEYWORDS = [
    "arbitration", "dispute", "jurisdiction", "governing law", "force majeure",
    ...
    for keyword in HIGH_RISK_KEYWORDS:
        if re.search(r'\b' + re.escape(keyword), clause_lower):
            found.append({
                "word": keyword,
                "risk_level": "HIGH",
                "reason": KEYWORD_REASONS.get(keyword, "High risk legal term"),
                "weight": 0.9,
            })
    for keyword in MEDIUM_RISK_KEYWORDS:
        if re.search(r'\b' + re.escape(keyword), clause_lower):
            found.append({
                "word": keyword,
                "risk_level": "MEDIUM",
                "reason": KEYWORD_REASONS.get(keyword, "Medium risk legal term"),
                "weight": 0.5,
            })
